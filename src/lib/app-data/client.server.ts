...
async function gatePost(
  ...
  if (!/^https?:\/\//i.test(base)) {
    throw new Error(`gate base must be absolute http(s) URL (got ${base})`);
  }
...
export async function callTool(
  ...
  if (!connectorType) {
    return {
      ok: false,
      data: null,
      errorMessage: "connectorType is required",
    };
  }

  const memoKey = safeMemoKey([
    toolName, args, connectorType, options?.connectorCatalogId ?? null, tokenIdentityKey(token),
  ]);
  ...
  if (connectorType === ConnectorType.Mcp && !options?.connectorCatalogId) {
    return { ok: false, data: null, errorMessage: "connectorCatalogId is required when connectorType is Mcp" };
  }
